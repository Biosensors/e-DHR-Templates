USE [BpmTemplate]
GO
/****** Object:  StoredProcedure [dbo].[DHR_get_equipment_check_summary_for_qc_confirmation_by_order_no]    Script Date: 31/8/2023 5:06:23 pm ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[DHR_get_equipment_check_summary_for_qc_confirmation_by_order_no]
	@route_sheet_tmpl_family varchar(100), -- DHR QC Inspection Route Sheet
	@order_no varchar(40)
AS
BEGIN
select 
...
[shift],cast([sequence] as int) [sequence],table_id,row_index,
[Equipment Name],
[Equipment ID],
...
from (
		...
		(select top 1  m1.action_on
			...
			(select value from [BsWebService].[dbo].bpm_process_detail where process_matrix_id=d.process_matrix_id and label='Shift') as [shift],
			...
			inner join [BsWebService].[dbo].bpm_process_detail d on 
				...
				--and t.name like 'DHR QC Inpection Route Sheet%'
				and 
		...
		as pivotTable
		order by [Original Completion Date],[Equipment Name]
END
